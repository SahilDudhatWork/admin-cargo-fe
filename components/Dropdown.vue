<template>
  <div>
    <button
      ...
      class="dropdown-list relative bg-white mx-0 text-[#5B638B] font-semibold rounded-lg text-base py-[14px] w-full xl:w-[382px] text-center inline-flex justify-between px-2"
      :class="
        isDropdown ? ' border-t border-r border-l rounded-none' : 'border'
      "
      type="button"
    >
    ...
    <div
      ...
      class="z-50 bg-white rounded-lg shadow xl:w-[382px] w-full dark:bg-gray-700"
    >
      <ul class="text-sm text-gray-700 dark:text-gray-200 cursor-pointer">
        <li v-for="(item, index) in items" :key="index">
          <a @click.prevent="selectItem(item, index)">
            <span
              class="block px-4 py-[16px] font-medium border border-gray-200"
            >
              {{ item.label }}
            </span>
          </a>
