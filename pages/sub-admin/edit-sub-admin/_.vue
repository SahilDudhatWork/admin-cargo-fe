    ...
    <div>
      <div class="flex items-center gap-3 mb-5">
        <h1 class="text-[#989898] font-normal text-[12px] cursor-pointer">
          <nuxt-link to="/sub-admin">SUB ADMIN</nuxt-link>
        </h1>
      ...
      <form class="space-y-4 md:space-y-6 mt-6" @submit.prevent="editSubAdmin">
        <div>
          <div
...
export default {
  ...
  computed: {
    ...mapGetters({
      getSingleSubAdminData: "sub-admin/getSingleSubAdminData",
      allRoleData: "role/getAllRoleData",
    }),
    roleOptions() {
      return this.allRoleData.map((role) => ({
  ...
  methods: {
    ...mapActions({
      updateSubAdmin: "sub-admin/updateSubAdmin",
      fetchSingleSubAdmin: "sub-admin/fetchSingleSubAdmin",
    ...
    getRoleValue(item) {
      ...
      this.formData.roleByAdmin = item.value;
    },
    async editSubAdmin() {
      try {
        this.errors = await this.$validateSubAdminForm({
          form: this.formData,
        ...
        const response = await this.updateSubAdmin(this.formData);
        this.$toast.open({
  ...
  async asyncData({ params, store, redirect }) {
    ...
    try {
      await store.dispatch("sub-admin/fetchSingleSubAdmin", {
        id: id,
      });
    } catch (error) {
  ...
  beforeMount() {
    this.formData = this.$lodash.cloneDeep(this.getSingleSubAdminData);
    const selectedRole = this.allRoleData.find(
      (role) => role._id === this.getSingleSubAdminData.roleByAdmin
    );
    if (selectedRole) {
      this.selectedLabel = selectedRole.roleTitle;
    }
  },
};
</script>
